...
class CreateController extends Controller
{
   public function index(){
      try {
         //code...
         return view('components.add-applicant');
      } catch (Exception $err) {
         return abort(500);
      }
   }
   ...
   public function action(Request $request) {
      ...
      try {
         ...
         {
            DB::rollBack();
            return redirect('/create')->withErrors($validator->messages())->withInput($request->all());
         }


         $fams = Family::create($family);
         $personal['family_id'] = $fams->id;
